...
// an array that stores the bounds of the
// glyphs that we're going to draw
static const GlyphBounds glyphs[] = { { 'c', 0.45, 0.5 }, { 'a', 0.45, 0.5 }, {
		'i', 0.2, 0.75 }, { 'r', 0.4, 0.5 }, { 'o', 0.44, 0.5 }, { 'm', 0.75,
		0.5 }, { '!', 0.2, 0.75 } };

// A *very* simple font that just draws a box for every glyph
class BoxFontFace: public Cairo::UserFontFace {
public:
	// Derived user font classes should have
	...
	virtual Cairo::ErrorStatus init(
			const Cairo::RefPtr<Cairo::ScaledFont>& /*scaled_font*/,
			const Cairo::RefPtr<Cairo::Context>& /*cr*/,
			Cairo::FontExtents &extents) {
		double max = 0;
		for (unsigned int i = 0; i < sizeof(glyphs) / sizeof(GlyphBounds);
				++i) {
	...
	virtual Cairo::ErrorStatus unicode_to_glyph(
			const Cairo::RefPtr<Cairo::ScaledFont>& /*scaled_font*/,
			unsigned long unicode, unsigned long& glyph) {
		glyph = 0;
		// yes this is a stupid an ineffienct way
		// to do this but we only have a few
		// glyphs and this is just demonstration code
		for (unsigned int i = 0; i < sizeof(glyphs) / sizeof(GlyphBounds);
				++i) {
			if (glyphs[i].glyph == unicode) {
				// glyph 0 is often a special
				// glyph-not-found value, so offset it by 1
				glyph = i + 1;
				break;
			}
		}
	...
	virtual Cairo::ErrorStatus render_glyph(
			const Cairo::RefPtr<Cairo::ScaledFont>& /*scaled_font*/,
			unsigned long glyph, const Cairo::RefPtr<Cairo::Context>& cr,
			Cairo::TextExtents& metrics) {
		// check that the glyph is in our table
		if (glyph >= 1 && glyph <= sizeof(glyphs) / sizeof(GlyphBounds)) {
			cr->set_line_width(0.05);
			// Need a negative Y value since the
			//text origin is at the bottom left point
			// and cairo's positive Y axis is
			// down and we want to draw up
			cr->rectangle(0.0, 0.0, glyphs[glyph - 1].width,
					-glyphs[glyph - 1].height);
			cr->stroke();
...
protected:
	// FontFace is a ref-counted object,
	// so the constructor should be protected so
	// it is not created without a refptr to
	// manage it. See the create() method
	BoxFontFace() :
			UserFontFace() {
	}
};
